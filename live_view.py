#!/usr/bin/env python3
"""Display-only live viewer: original clip + boxes + synced top-down minimap.

Draws player/ball boxes on the ORIGINAL clip with a top-down minimap docked
right (moving in lockstep) and a stats panel below it. Nothing is saved.

Reads only CSVs the pipeline already produced — never re-runs a tracker or YOLO
— so it is fast and every overlay stays frame-synced:
...
The ball box is redrawn on the source frame from the ball CSV, so the pre-burned
annotated video is never needed.

Usage (from the project root):
    python live_view.py --video data/Input_video2.mp4

CSV paths default to <stem>-based names under ``--output``; each is overridable.
...
# Project root on sys.path so utils.*/tracking.* import regardless of cwd.
PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))
if PROJECT_ROOT not in sys.path:
    sys.path.insert(0, PROJECT_ROOT)

# player_analysis is reused only for its px->metre conversion (_load_and_convert)
# and the ITF court constants; the minimap is drawn natively with OpenCV.
# Side effect: importing it pins matplotlib to the headless "Agg" backend (no-op
# here — no figures are created).
...
# ── layout constants ────────────────────────────────────────────────────────────
# RENDER and WINDOW size are decoupled: the canvas is composed at RENDER_WIDTH so
# all text rasterises large and crisp, then area-downscaled ONCE to WINDOW_WIDTH
# for display. A smaller window thus costs no text quality (it shrinks already-
# crisp pixels) — unlike the old single --max-width that drove both.
...
MINI_FRAC = 0.50         # minimap's share of the right-column height (stats gets
                         # the rest: 8 category rows + totals + footer need the room)
FONT      = cv2.FONT_HERSHEY_SIMPLEX
# Heavier font for on-VIDEO labels — they sit small over a busy court where SIMPLEX
# reads thin. Panel/minimap keep SIMPLEX (denser text on a controlled dark bg).
FONT_VIDEO = cv2.FONT_HERSHEY_DUPLEX

# Supersample factor for the side panels: rendered at SS× then INTER_AREA-shrunk
# in compose(), so thin Hershey strokes come out anti-aliased. The extra pixel
# work on a ~380-px column is sub-millisecond.
SS = 2

# On-video label style. Drawn at DISPLAY resolution (after the resize) so strokes
# stay crisp; a dark semi-transparent plate keeps the coloured text legible.
...
# Brighter tints for panel/legend TEXT (the pure box colours are too dark on the
# warm panel bg); on-video boxes keep the pure _BOX_COLORS.
...
# Shot-type legend, drawn INSIDE the minimap in the free green run-off strip left
# of the court (left sideline ~x=142 px in a 380-px panel, so x<~135 is clear).
# One row per entry: symbol + shot-type label.
...
# Stable category order so legend/stats rows never reshuffle as shots accumulate.
_LEG_ORDER = ["forehand", "backhand", "slice", "dropshot",
              "lob", "serve", "smash", "unknown"]

# On-VIDEO player box colours, matching playerTracking._draw_players (P1 green,
# P2 red). Minimap dots reuse these via _DOT_BGR; the "P1"/"P2" labels carry id.
...
def _resolve(path: str) -> str:
    ...
    Bridges the on-disk ``input_video2_court.csv`` vs the stem-derived
    ``Input_video2_court.csv`` mismatch on case-sensitive filesystems. Falls back
    to the original ``path`` so the caller's "not found" message still makes sense.
    """
...
def derive_default_paths(video: str, output: str) -> dict:
    """Default CSV paths per modality under ``output``, keyed by video stem.

    Mirrors each producer's own default location/naming.
    """
...
def load_shot_markers(shots_csv: str, minimap: "Minimap",
    ...
    Returns ``[(frame, pid, category, (px, py)), ...]`` with (px, py) in panel
    pixels via ``minimap._to_px`` (same transform as the live dots, so they share
    one coordinate system). Placed at the player's FEET (``player_x_m/y_m``) to
    match shot_hitmap.png. ``bounds`` = (x_lo, x_hi, y_lo, y_hi) walkable-area
    limits in metres. NaN positions and out-of-bounds shots are skipped; multiple
    shots per frame are all kept.
    """
    import pandas as pd
...
# Dot colours (BGR), matching _BOX_COLORS (P1 green, P2 red) so the minimap reads
# coherently with the on-video boxes.
...
def _hex_to_bgr(hexstr: str) -> tuple[int, int, int]:
    """'#rrggbb' -> (B, G, R) for OpenCV; grey on a malformed value."""
    s = str(hexstr).lstrip("#")
    if len(s) != 6:
        return (127, 127, 127)
...
# Category -> BGR, converted ONCE from the shared hex palette so live markers
# match shot_hitmap.png exactly (single source of colour truth).
_CAT_BGR = {cat: _hex_to_bgr(hx) for cat, hx in SHOT_CATEGORY_COLORS.items()}
_CAT_FALLBACK_BGR = (127, 127, 127)   # grey, mirrors shot_analysis' fallback

# Shot marker geometry: hollow and ~1 px larger than the 6-px FILLED player dot,
# so it frames the spot without dwarfing it.
...
def _draw_shot_marker(img, center, pid, color_bgr,
    ...
    """Hollow, category-coloured marker; SHAPE encodes the player (P1 circle,
    P2 up-triangle, as in shot_hitmap.png).

    A near-black ring 1 px wider is stroked under it so it stays legible on the
    green court and over the white lines.
    """
    ...
    else:   # P1 and any unknown id -> hollow circle
...
class Minimap:
    ...
    The court (ITF singles dimensions + walkable run-off, constants from
    player_analysis) is rasterised ONCE into a base image; each frame copies that
    base and stamps the dots. Sub-millisecond and O(1) memory per frame, so it
    ...
    def __init__(self, size: tuple[int, int]):
        self.w, self.h = size
        # Supersample scale (panel built at SS× on-screen width): marker/dot radii
        # and the legend scale by this to hold their on-screen size after the
        # INTER_AREA downscale in compose().
    ...
    def _draw_base(self) -> np.ndarray:
        ...
        # (Legend is stamped per-frame in render(), not baked into the base.)
        return img

    def _draw_legend(self, img, present_cats) -> None:
        """Stamp the shot legend in the free green strip left of the court.

        One row per category in ``present_cats`` (swatch + near-WHITE label so
        dark hues stay legible), in stable _LEG_ORDER, then a P1=circle/P2=triangle
        shape key, over a darkened backdrop. Geometry/fonts scale by ``s`` for the
        SS× canvas, smoothing after the INTER_AREA downscale in compose().
        """
        s = self.w / PANEL_W
        def S(v): return int(round(v * s))
        leg_fs = _LEG_FS * 1.2 * s
        ...
        n = len(rows) + 2 + 1   # categories + 2 shape-key rows + 1 header
        # Backdrop sized to the column; clamped to the panel just in case.
        ...
        for cat in rows:
            ...
            cv2.putText(img, cat, (lbl_x, y), FONT, leg_fs, _PANEL_FG, th,
                        cv2.LINE_AA)
            y += row_h
        # player shape key (neutral white so only the SHAPE carries identity)
    ...
    def render(self, positions: dict, shot_markers=None,
        ...
        """Compose one BGR minimap frame.

        ``positions`` = {pid: (xm, ym)} live dots; ``shot_markers`` =
        [(pid, category, (px, py)), ...] accumulated so far; ``present_cats``
        drives the in-minimap legend. Markers are stamped FIRST (history layer)
        and live dots LAST (present layer), so a dot is never hidden; markers are
        hollow so an overlapping dot still shows its ring. Legend stamped last to
        stay readable over everything.
        """
        img = self._base.copy()
        # Scale marker/dot geometry to the SS× canvas (constant on-screen size
        # after compose()'s downscale).
        s = self._s
...
def render_stats_panel(size: tuple[int, int], counts: dict,
    ...
    ``counts`` = {pid: {category: n}}; ``cats`` is the fixed ordered category list
    so rows never reshuffle. Each column lists one counter per category then the
    per-player total; the footer shows rally length = total shots by both players.
    ``ended`` switches the title to FINAL and adds the "press q" footer.
    """
    w, h = size
    # Rendered at SS× then area-downscaled in compose, so derive ``s`` from the
    # incoming width and express all sizes as NATIVE units * s (resolution-
    # agnostic). Labels are near-white with the colour carried ONLY in the swatch,
    # so every row clears a contrast floor regardless of hue.
    ...
    th_body = max(1, S(1))                            # ~2 px at SS=2
    th_bold = max(2, S(1.5))

    # Title band + separator rule.
    ...
    # Two equal columns headed by each player in their brightened tint.
    ...
    # Row pitch is FIT-DRIVEN: divide header->footer space by the row count
    # (categories + total) so everything stays above the footer, clamped to a
    # readable range. Font tracks the pitch with a legible floor, so a short panel
    # shrinks text rather than colliding.
    ...
    # per-player total flows after the last category (n_rows reserved its slot),
    # then a thin rule separates it from the footer band.
...
def draw_label(img: np.ndarray, text: str, anchor: tuple[int, int],
    ...
    """Draw ``text`` over a dark semi-transparent plate, in place.

    Call on the ALREADY-RESIZED display image so glyphs rasterise once at final
    size (no softening downscale). ``anchor`` is a bbox-edge point; ``above=True``
    puts the plate just above it (framing a box top-left). The plate is clamped to
    the image so an edge label never indexes out of range.
    """
    ...
    x1, y1 = x0 + pw, y0 + ph
    roi = img[y0:y1, x0:x1]
    if roi.size:   # blend a dark plate behind the glyphs
...
def compose(frame: np.ndarray, minimap: np.ndarray, stats: np.ndarray,
    ...
    Overlays are drawn HERE on the resized video (not the source frame) so label
    strokes stay crisp instead of softening under the source->display downscale.
    ``player_overlays`` = [(pid, x, y, w, h, color), ...] (box + "P#" label),
    ``ball_overlay`` = (x, y, w, h) (box, no label), both in SOURCE-pixel coords,
    scaled by the frame's resize factor. The minimap/stats panels arrive at SS×
    and are area-downscaled here for smooth text.
    """
    h0, w0 = frame.shape[:2]
    vw = int(round(w0 * (disp_h / h0)))
    # INTER_AREA is the alias-free filter for this ~0.5× downscale; INTER_LINEAR
    # softened court and box edges.
    video = cv2.resize(frame, (vw, disp_h), interpolation=cv2.INTER_AREA)

    # Scale source-pixel coords to the resized video, then draw boxes + labels.
    ...
    canvas[:, :vw] = video
    # Area-downscale the SS× panels to fit their exact sub-regions (also avoids
    # any assembly shape mismatch).
...
def _fit_to_screen(canvas: np.ndarray, window_width: int) -> np.ndarray:
    """Downscale the whole canvas (aspect-preserved) to ``window_width``.

    The ONE place render resolution becomes window size: a single INTER_AREA step
    shrinking RENDER_WIDTH-composed, already-crisp pixels, so a smaller window
    costs no text quality. No-op when the canvas already fits; ``window_width<=0``
    disables scaling (show at full render resolution).
    """
...
def _screen_size(default=(1920, 1080)) -> tuple[int, int]:
    """(width, height) of the primary screen in px; ``default`` if unknown (Windows-only API)."""
    try:
        import ctypes
...
def _center_window(win_name: str, w: int, h: int) -> None:
    """Centre an OpenCV window's w×h box on the primary screen."""
    sw, sh = _screen_size()
    x = max(0, (sw - w) // 2)
    y = max(0, (sh - h) // 2)
...
def _generate_analysis_figures(players_csv, court_csv, ball_csv, output_dir,
                               fps, min_area, anchor):
    """Regenerate the three summary figures in-process, headless.

    heatmap_combined.png + zones.png (player_analysis) and shot_hitmap.png
    (shot_analysis) only — the slow minimap.gif and per-shot PNGs are skipped.
    Agg backend, so nothing pops up. Raises on any failure so the caller aborts
    (the figures are required for the end-of-clip summary).
    """
    ...
    # --- player analysis: combined heatmap + court zones (no gif) ---
...
# The three figures tiled when the clip ends, with their fixed paths relative to
# the --output base dir (see player_analysis.py / shot_analysis.py).
_SUMMARY_FIGS = [
...
def _summary_panel(title: str, img, panel_h: int, bar_h: int = 34) -> np.ndarray:
    """One labelled summary column: title bar over the figure (or a 'not found'
    placeholder), scaled to ``panel_h`` total height."""
    fig_h = panel_h - bar_h
    if img is None:                      # missing/unreadable -> placeholder
...
def _show_summary_window(output_dir: str, window_name: str,
    ...
    """Open a separate, centred window tiling heatmap | zones | shot hitmap.

    Returns True if at least one figure was found. Missing figures become a
    ...
    if max_width and board.shape[1] > max_width:     # fit to a sane width
...
def run(video, players_csv, court_csv, ball_csv, min_area, anchor,
    ...
    # Solve the RENDER height so the canvas width (video + PANEL_W) == render_width
    # while the video keeps its aspect ratio. The shown window is a separate,
    # smaller size (canvas area-downscaled once at display time):
    #   vw + PANEL_W = render_width, vw = src_w*disp_h/src_h
    #   => disp_h = (render_width - PANEL_W) * src_h / src_w.
    ...
    # --- regenerate the summary figures up front so they always reflect the
    # CURRENT CSVs (no manual analysis run). Abort on failure — the summary needs
    # them. ---
    ...
    # --- per-frame overlays, indexed by absolute frame number ---
    ...
    # Walkable-area bounds (1 m of slack), same guard as player_analysis._save_minimap.
    ...
    # Built at SS× so court lines + legend smooth after compose()'s downscale.
    # _to_px (and the markers pre-resolved through it) inherit the SS× scale, so
    ...
    if shots_csv and os.path.exists(shots_csv):
        ...
        except Exception as e:   # a bad CSV must never break playback
    ...
    # FIXED legend: all categories in shots.csv, computed once so the full key
    # shows from frame 0 (it does NOT build up). None when there are no shots.
    legend_cats = {cat for _f, _pid, cat, _c in shot_markers} or None
    # Ordered category list for the stats columns (only types that occur).
    stat_cats = [c for c in _LEG_ORDER if legend_cats and c in legend_cats]

    def minimap_positions(frame_idx: int) -> dict:
        """{pid: (xm, ym)} for players present and in range at frame_idx."""
        out = {}
    ...
    if not fps or not np.isfinite(fps) or fps <= 0:
        fps = 30.0
    frame_period = 0.915 / fps   # seconds-per-frame (0.915 fudge trims render lag)

    win = "Tennis Live View  (q to quit)"
    # Frames are pre-downscaled to window_width, so the shown image is 1:1 crisp.
    cv2.namedWindow(win, cv2.WINDOW_NORMAL)
    if window_width and window_width > 0:
        # Size the window at window_width, preserving the canvas aspect ratio
        # (canvas is vw + PANEL_W wide by disp_h tall), centred on screen.
    ...
    idx = 0
    # shot_markers is frame-sorted, so one advancing pointer (next_shot) reveals
    # each marker as its frame is reached and keeps it shown to clip end
    # (O(total shots), not per frame).
    active_markers = []     # [(pid, cat, center), ...] — frames already crossed
    next_shot = 0
    # Per-player tally, bumped exactly when a shot is revealed so the panel
    # counters stay in step with the markers.
    counts = {1: {}, 2: {}}
    last_canvas = None      # final composite to freeze on after the clip
    try:
        while True:
            ...
            # resized video so the text stays crisp.
            player_overlays = []
            ...
            # bumping that player's per-category counter as its marker appears.
            ...
            # legend_cats is FIXED (full key from frame 0). Panels render at SS×
            # and area-downscale in compose() for smooth text.
            ...
            # Downscale the high-res canvas to the window size ONCE here.
            cv2.imshow(win, _fit_to_screen(last_canvas, window_width))

            # Wait only the time LEFT in this frame's period (after processing) so
            # render overhead doesn't drag playback below real time.
        ...
        # --- clip finished: FREEZE on the last frame, recomposed with the final
        # tally, and block until 'q'. ---
        if last_canvas is not None:
            ...
            # Reuse the last video frame in last_canvas's left part; only the
            # panels are re-stamped (area-downscaled, same as compose).
            ...
            # Separate window tiling the whole-clip analysis figures.
...
def main() -> None:
    ...
    # Players + court are required; ball and shots CSVs are optional (see run()).
