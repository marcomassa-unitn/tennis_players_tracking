...
class CourtDetector:
    def __init__(self, video_path):
        ...
        self.all_corners = []  # Store all detected corners across frames
    ...
    def find_singles_corners(self, lines, frame_shape):
        ...
        # --- INIZIO MODIFICA ---
        # Calcolo dei limiti della ROI (Region of Interest).
        # Mantenendo il 95% della larghezza, si taglia il 2.5% a destra e sinistra.
        # Mantenendo l'80% dell'altezza, si taglia il 10% sopra e sotto.
        min_x, max_x = w * 0.025, w * 0.975
        min_y, max_y = h * 0.10, h * 0.90
        # --- FINE MODIFICA ---
        ...
        for line in lines:
            ...
            # --- INIZIO MODIFICA ---
            # Se uno dei due punti della linea esce dai limiti, la linea viene ignorata
            if not (min_x <= x1 <= max_x and min_x <= x2 <= max_x and
                    min_y <= y1 <= max_y and min_y <= y2 <= max_y):
                continue
            # --- FINE MODIFICA ---
    ...
    def run(self):
        ...
        while True:
            ...
            if corners:
                self.all_corners.extend(corners)

            # Draw all persisted corners on the frame
            for pt in self.all_corners:
...
if __name__ == "__main__":
    detector = CourtDetector("data/input_video2.mp4")
    detector.run()
